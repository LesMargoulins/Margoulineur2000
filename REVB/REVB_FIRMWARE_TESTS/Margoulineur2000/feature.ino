...
void   batStatus()
{
    ...
    while (digitalRead(encButton))
    {
      ...
      lcd.setCursor(0, 1);
      lcd.print(F("V : "));
      int batVoltage = (map(analogRead(BatSense), 0, 1023, 0, 5000));

      Serial.println(analogRead(BatSense));
      Serial.println(batVoltage); //in mV
...
/*
      ...
      lcd.setCursor(9 ,1);
      lcd.print(F("% : "));
      lcd.print(map(map(analogRead(BatSense), 0, 1023, 0, 859), 0,470, 0, 100));
*/
      delay(200);
    }
    lcd.clear();
